...
function [t,V] = stmolisyn(dt,tfin,gmax,gT)
...
for j=2:Nt
    ...
    gsyn = gmax*(mod(t(j)-dt/2,gT)<1);
    
    top = 2*Cm*V(j-1)/dt + g.Na*m*h*E.Na + g.K*n*E.K + g.A*a*b*E.K + g.L*E.L;
    bot = 2*Cm/dt + g.Na*m*h + g.K*n + g.A*a*b + g.L + gsyn;
